#!/bin/sh
# =============================================================================
# shastenm dotfiles — full desktop installer — FreeBSD 14
#
# Repos:
...
# Note: FreeBSD /bin/sh — no set -u (undefined var checks differ).
# Pipefail is bash-only; use explicit exit checks instead.
set -e
...
# 1. System update
# =============================================================================
_section "Step 1 — System update (pkg)"
...
# 2. Core pkg packages
# =============================================================================
_section "Step 2 — Core packages"

# FreeBSD pkg names for the equivalents of the Slackware core list.
# fontconfig and curl/wget are usually already present but listed for safety.
CORE_PKGS="git curl wget fontconfig neovim jq"
...
for pkg in $CORE_PKGS; do
    if _pkg_installed "$pkg"; then
        ...
        sudo pkg install -y "$pkg" || _warn "$pkg install failed — check pkg repos"
    fi
done
...
# 3. Xorg
# =============================================================================
_section "Step 3 — Xorg"

if _pkg_installed "xorg"; then
    _info "xorg already installed"
else
    _info "Installing xorg..."
    sudo pkg install -y xorg || _warn "xorg install failed — install manually if X is already set up"
fi

# =============================================================================
# 4. Rice tooling via pkg
# =============================================================================
_section "Step 4 — Rice / desktop packages"

# FreeBSD pkg equivalents of the SBo list.
# spectrwm and dzen2 are available in ports/pkg on FreeBSD.
RICE_PKGS="
    spectrwm
    nitrogen
    picom
...
for pkg in $RICE_PKGS; do
    if _pkg_installed "$pkg"; then
        ...
        _info "Installing $pkg..."
        sudo pkg install -y "$pkg" || _warn "$pkg install failed — install manually or check ports"
    fi
done

# =============================================================================
# 5. dzen2 fallback — build from source if pkg install failed
# =============================================================================
_section "Step 5 — dzen2 source build (fallback)"

if ! command -v dzen2 >/dev/null 2>&1; then
    _warn "dzen2 not found after pkg — building from source"
    # Build deps
    sudo pkg install -y libX11 libXft libXinerama libXpm gmake || true
    mkdir -p "$BUILD_DIR" && cd "$BUILD_DIR"
    [ -d dzen ] || git clone https://github.com/robm/dzen.git
    cd dzen
    # dzen2 Makefile uses GNU make; on FreeBSD that's gmake
    gmake || make
    sudo cp dzen2 /usr/local/bin/dzen2
    sudo chmod +x /usr/local/bin/dzen2
    _info "dzen2 installed from source"
else
...
_bak() {
    local t="$HOME/$1"
    if [ -e "$t" ] && [ ! -L "$t" ]; then
        _warn "Backing up $t -> ${t}.bak"
        mv "$t" "${t}.bak"
    fi
}

# FreeBSD default shell config files
_bak ".bashrc"
_bak ".bash_profile"
_bak ".profile"
...
# On FreeBSD, bash may not be the login shell. We target ~/.bashrc but also
# drop a note about csh/sh users needing manual setup.
BASHRC="$HOME/.bashrc"
...
for s in $SCRIPTS; do
    if [ -f "$SPECTRWM_CONF/$s" ]; then
        chmod +x "$SPECTRWM_CONF/$s"
    else
...
if [ -f "$SWCONF" ]; then
    sed -i '' 's|^\(bar_action[[:space:]]*=.*\)|# \1|' "$SWCONF"
    if grep -q '^bar_enabled' "$SWCONF"; then
        sed -i '' 's|^bar_enabled[[:space:]]*=.*|bar_enabled = 0|' "$SWCONF"
    else
...
# Note: xfsettingsd (XFCE) removed — not applicable on FreeBSD bare spectrwm setup

# =============================================================================
# 14. Fix dzen2 bar — replace ubuntu-mono font with Mononoki Nerd Font
# =============================================================================
...
if [ -f "$DZEN_BAR" ]; then
    # BSD sed requires '' for in-place with no backup
...
_info "  2. Reload shell:    source ~/.bashrc   (or re-login if using sh/csh)"
_info "  3. Start spectrwm:  startx   — or select it from your display manager"
printf "\n"
_warn "If bash is not your login shell, add starship init manually to your shell's rc file"
_warn "clipmenu may not be in pkg — check ports x11/clipmenu if missing"
_warn "eza may be listed as 'eza' or 'rust-eza' in pkg depending on FreeBSD version"
